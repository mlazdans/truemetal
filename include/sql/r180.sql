...
DROP TRIGGER IF EXISTS `comment_connect_trigger2`;
...
ALTER TABLE `res` ADD INDEX ( `login_id` );

/* Article */
ALTER TABLE `article` CHANGE `art_id` `art_id` INT UNSIGNED NOT NULL AUTO_INCREMENT;
ALTER TABLE `article` ADD `res_id` INT UNSIGNED NULL AFTER `art_id` ,
ADD UNIQUE (
`res_id`
);
ALTER TABLE `article` ADD FOREIGN KEY ( `res_id` ) REFERENCES `res` (
`res_id`
...
ALTER TABLE `article` ADD FOREIGN KEY ( `login_id` ) REFERENCES `logins` (
`l_id`
...
ALTER TABLE `forum` ADD FOREIGN KEY ( `res_id` ) REFERENCES `res` (
`res_id`
) ON DELETE SET NULL ON UPDATE SET NULL ;
...
ALTER TABLE `comment` ADD FOREIGN KEY ( `res_id` ) REFERENCES `res` (
`res_id`
) ON DELETE SET NULL ON UPDATE SET NULL ;
...
update `comment` set login_id=NULL  where `login_id` not in (select l_id from logins);
ALTER TABLE `comment` ADD FOREIGN KEY ( `login_id` ) REFERENCES `logins` (
`l_id`
) ON DELETE SET NULL ON UPDATE SET NULL ;
...
/* Triggers */
-- TRIG_res_comment_trigger1_AFTER_INSERT.sql
-- TRIG_res_comment_trigger2_AFTER_DELETE.sql
-- TRIG_res_vote_trigger1_AFTER_INSERT.sql
-- TRIG_res_vote_trigger2_AFTER_DELETE.sql


CALL res_update_meta_all();
