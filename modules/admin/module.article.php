...
function article_error($msg, &$template) {
...
/* save */
	...
	if($id = $article->save($art_id, $_POST['data'])) {
		...
		article_error($module->error_msg, $template);
} elseif($action == 'art_new') {
// jauns
	if($art_modid) {
		...
		// editor
		set_editor($template, 'article_editor', 'articles');
	} else {
...
// saraksts
	if(!$art_id)
	{
		...
		{
			++$article_count;
			$template->set_var('article_nr', $article_count);
			$template->set_var('art_name', $item['art_name']);
			$template->set_var('art_id', $item['art_id']);
			$template->set_var('module_id', $item['module_id']);

			$template->set_var('art_color_class', 'box-normal');
			...
			$template->parse_block('BLOCK_articles', TMPL_APPEND);
		} // foreach articles
		$template->set_var('article_count', $article_count);
	} else {
// redigeet
		$art = $article->load($art_id, 0, ARTICLE_ALL, ARTICLE_ALL);
		$template->enable('BLOCK_article_edit');

		// editor
		set_editor($template, 'article_editor', 'articles', $art_id);

		$template->set_array($art);
		//$template->set_var('art_comments', $art['art_comments'] == 'Y' ? ' checked' : '');

		if($art['art_type'] == ARTICLE_TYPE_OPEN)
			$template->set_var('art_type_o', ' selected');
		elseif($art['art_type'] == ARTICLE_TYPE_REGISTRATED)
			$template->set_var('art_type_r', ' selected');

		if($art['art_comments'] == ARTICLE_COMMENTS)
			$template->set_var('art_comments_y', ' selected');
		else
			$template->set_var('art_comments_n', ' selected');

		if($art['art_visible'] == ARTICLE_VISIBLE)
			$template->set_var('art_visible_y', ' selected');
		else
			$template->set_var('art_visible_n', ' selected');

		if($art['art_active'] == ARTICLE_ACTIVE)
			$template->set_var('art_active_y', ' selected');
		else
			$template->set_var('art_active_n', ' selected');
