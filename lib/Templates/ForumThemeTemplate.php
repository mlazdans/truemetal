	...
	{ ?>
		<div class="forum-theme">
			<div class="forum-theme-main">
				<div class="forum-theme-name"><a href="<?=$this->res_route ?>"><b><?=specialchars($this->res_name) ?></b></a></div>
			...
			<div class="forum-theme-info">
				<? if($this->profile_link_enabled) { ?>
					<a href="/user/profile/<?=$this->l_hash ?>/" class="ProfilePopup" data-hash="<?=$this->l_hash ?>" style="color: white;"><?=specialchars($this->res_nickname) ?></a>, <?=$this->res_date ?>
				<? } else { ?>
					<?=specialchars($this->res_nickname) ?>, <?=$this->res_date ?>
				<? } ?>
			</div>
		</div><?
	}
