...
SELECT
	...
	CONCAT(art_intro, ' ',  art_data) AS doc_content,
...
JOIN comment_meta ON (cm_table = 'article') AND (cm_table_id = art_id)
WHERE
...
FROM
	forum f
JOIN comment_meta ON (cm_table = 'forum') AND (cm_table_id = forum_id)
