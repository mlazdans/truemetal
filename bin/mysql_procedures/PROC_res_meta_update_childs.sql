...
BEGIN
	...
	INSERT INTO res_meta (
		...
		COUNT(CASE WHEN parent.res_kind = 3 THEN parent.res_id ELSE NULL END) AS comment_count,
		MAX(parent.res_entered) AS child_last_date,
		MAX(CASE WHEN parent.res_kind = 3 THEN parent.res_entered ELSE NULL END) AS comment_last_date
	FROM res r
